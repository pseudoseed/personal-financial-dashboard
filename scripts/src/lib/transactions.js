...
exports.downloadTransactions = downloadTransactions;
const plaid_1 = require("plaid");
const plaidClient = new plaid_1.PlaidApi(new plaid_1.Configuration({
...
async function downloadTransactions(prisma, account, force) {
...
async function handleRegularTransactions(prisma, account, force) {
    let allTransactions = [];
    let hasMore = true;
    // If forcing a full refresh, ignore the existing cursor
    let cursor = force ? undefined : account.plaidSyncCursor || undefined;
    ...
    while (hasMore) {
        ...
        const addedTransactions = response.data.added.filter((tx) => tx.account_id === account.plaidId);
        const modifiedTransactions = response.data.modified.filter((tx) => tx.account_id === account.plaidId);
        const removedTransactions = response.data.removed.filter((tx) => tx.account_id === account.plaidId);
        ...
        for (const modifiedTx of modifiedTransactions) {
            ...
            await prisma.transaction.update({
                ...
                data: {
                    ...
                    personalFinanceCategory: modifiedTx.personal_finance_category?.primary || null,
    ...
    if (allTransactions.length > 0) {
        await prisma.$transaction(allTransactions.map((transaction) => {
...
async function handleInvestmentTransactions(prisma, account) {
    const endDate = new Date().toISOString().split("T")[0];
    const startDate = new Date();
    startDate.setMonth(startDate.getMonth() - 24); // Get 24 months of history
    const startDateStr = startDate.toISOString().split("T")[0];
    let allInvestmentTransactions = [];
    let allSecurities = [];
    let hasMore = true;
    let offset = 0;
    const PAGE_SIZE = 500;
    try {
        ...
        if (allInvestmentTransactions.length > 0) {
            await prisma.$transaction(allInvestmentTransactions.map((transaction) => {
                const security = transaction.security_id
                    ? allSecurities.find((s) => s.security_id === transaction.security_id)
                    : null;
